...
def eleven_text_to_speech(api_key: str, transcript_content: str, transcript_file: str, model_id: str):
    if not api_key:
...
def eleven_text_to_speech_with_effects(api_key: str, transcript_content: str, transcript_file: str, model_id: str = "eleven_v2")-> tuple[bytes, bytes] | None:
    ...
    # Create background sound effects
    sound_effect_description =  "cafe ambience with light chatter, coffee machine sounds, waiter taking orders"

    background_sfx = client.text_to_sound_effects.convert(  # type: ignore
        text= sound_effect_description,
        output_format="mp3_44100_128",
        loop=True,
        duration_seconds=45
    )

    background_bytes = b"".join(background_sfx)

    return full_audio, background_bytes
