...
def compute_jer(ref_segments: list[dict], hyp_segments: list[dict]) -> dict:
    ...
    for speaker in all_speakers:
        ref_intervals = [
            (seg["start"], seg["end"]) for seg in ref_segments if seg["speaker"] == speaker
        ]
        hyp_intervals = [
            (seg["start"], seg["end"]) for seg in hyp_segments if seg["speaker"] == speaker
        ]
